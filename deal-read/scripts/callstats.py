...
def main():
    ...
    rep_tokens = [t for t in re.findall(r"[a-z0-9]+", rep_key) if len(t) >= 3]
    ...
    def is_rep(speaker):
        ...
        speaker_tokens = set(re.findall(r"[a-z0-9]+", speaker_key))
        return bool(rep_key) and (
            rep_key in speaker_key
            or any(token in speaker_tokens for token in rep_tokens)
            or any(
                rep_token.startswith(speaker_token)
                for rep_token in rep_tokens
                for speaker_token in speaker_tokens
                if len(speaker_token) >= 3
            )
        )
