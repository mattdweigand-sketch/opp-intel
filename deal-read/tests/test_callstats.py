...
def main():
    ...
    # Fixture 5: Zoom often uses shortened speaker labels. The first-name token
    # should still count as the rep, without fuzzy matching arbitrary names.
    r = run("Matthew Weigand", [
        {"text": "Matt: What is your budget?"},
        {"text": "Buyer: Five dollars."},
    ])
    ok &= check("F5: short label rep_words > 0", r["rep_words"] > 0)
    ok &= check("F5: short label rep_questions 1", r["rep_questions"] == 1)
