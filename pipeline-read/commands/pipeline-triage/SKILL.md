...
2. Run the pipeline exactly as `$ENGINE/SKILL.md` describes:
   ...
   - §4 roll up once with `$ENGINE/scripts/rollup.py`. **Set `"mode":"triage"` in the rollup bundle.**
3. Present the **§5 triage brief** (riskiest first, on-track, where-you're-blind, your move this week).
...
`/pipeline-triage` runs the current fiscal quarter. `--next-quarter` / `--window current_quarter|next_quarter|30d`
