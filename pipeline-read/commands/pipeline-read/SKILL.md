---
name: pipeline-read
description: Ranked forecast-risk read across a rep's whole pipeline - riskiest deals first, each with its dominant risk (cited) and the single next move, plus a computed-inputs audit footer. Thin command frontend over the shared pipeline-read engine, run in read mode. Standard mode is fast and bulk-first; --deep-search opts into per-deal search agents across Salesforce, Gmail, Google Calendar, Zoom, Slack, and Google Drive within profile caps. Trigger on "/pipeline-read", "/pipeline-read --next-quarter", "/pipeline-read --deep-search", "read my pipeline", "which of my deals are most at risk", "what should I work this week across my deals". For the forecast-call view (the number, category rollup, keep/downgrade labels, movement vs a prior snapshot) use /pipeline-forecast instead. Per-rep, live connectors, read-only, no writes. Do NOT use for a deep read of one named deal (that is deal-read) or another rep's pipeline.
---
...
2. Run the pipeline exactly as `$ENGINE/SKILL.md` describes:
   - §1 resolve scope and list the in-scope opps (`$ENGINE/scripts/plan.py`, `mode:"pipeline"`;
     include `run_depth:"deep_search"` only when `--deep-search` is passed).
...
choose the window. `--deep-search` opts into the slower per-deal search-agent path. `--internal auto|off|force`
and `--internal-window 30d` tune internal evidence (default is `auto`: mapped rooms and linked docs only).
The forecast-only knobs (`--posture`,
